...
- **実装中に仕様と実態のズレを発見した場合、完了報告の「仕様フィードバック」に必ず記録する。以下の分類で報告すること：**
  - `[SPEC_GAP]` — 仕様に書かれていないが実装上必要だった判断
  - `[SPEC_WRONG]` — 仕様の記載が技術的に不可能または不適切だった
  - `[SPEC_AMBIGUOUS]` — 仕様が曖昧で解釈が必要だった

> 設計根拠: 仕様フィードバックの3分類は、コンテキストエンジニアリングの
> 「AIの失敗モードがどのコンテキストの欠如を示すか」の分類に基づく。
> 「AIエージェントが仕様を誤って実装したとき、その失敗モードはどのコンテキストが
> 欠けていたかを明らかにする」（WeBuild-AI, 2025）
...
コミット: [コミットハッシュ]
仕様フィードバック:
  - [SPEC_GAP] {内容}（該当なしの場合は「なし」）
  - [SPEC_WRONG] {内容}（該当なしの場合は「なし」）
  - [SPEC_AMBIGUOUS] {内容}（該当なしの場合は「なし」）
備考: [その他気づいた点があれば]
```

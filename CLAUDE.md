...
- 仕様はLiving Spec（spec-anchored）。実装で判明した事実は仕様に書き戻す
- PMは必須確認チェックリストに従ってPOに質問する
- ユースケース単位で1つずつ /orchestrate を回す。複数同時進行しない

> v5 設計根拠:
> - arXiv 2602.00180 のSDDの3レベル分類で spec-anchored に位置づけ
> - GitHub Spec Kit の Bidirectional Feedback + Tessl の情報アーキテクチャ
> - Thoughtworks「SDDはウォーターフォール回帰ではなく、フィードバックループの短縮」
...
> 設計根拠: マイクロスペック × イテレーティブSDD。
> 「アジャイルのユーザーストーリーが巨大な要件書を置き換えたのと同じように、
> 単一機能に絞った仕様書で高速に回す」（AWS）
> 「SDDはウォーターフォール回帰ではなく、フィードバックループの短縮」（Thoughtworks, 2025）
...
3. 前のユースケースで判明した事実は、次のユースケースの要件定義に反映する
